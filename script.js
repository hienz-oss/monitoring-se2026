...
function renderFilter() {
  ...
  const pmlList = [...new Set(
    allData.map(item => item.NAMA_PML).filter(Boolean)
  )];
  ...
  pmlList.forEach(pml => {
    const option = document.createElement("option");
    option.value = pml;
    option.textContent = pml;
    pmlFilter.appendChild(option);
  });
}
...
function getFilteredData() {
  ...
  const pml = document.getElementById("pmlFilter").value;
  ...
  return allData.filter(item => {
    const matchSearch =
      ...
      String(item.NAMA_PML || "").toLowerCase().includes(search);

    const matchPpl =
      !ppl || item.NAMA_PPL === ppl;

    const matchPml =
      !pml || item.NAMA_PML === pml;
    ...
    return matchSearch && matchPpl && matchPml && matchStatus;
  });
}
...
function renderRegionInfo() {
  ...
  document.getElementById("regionPml").textContent =
    formatNumber(totalPml);
}
...
function renderDashboard() {
  ...
  document.getElementById("totalSubmit").textContent =
...
function renderTopDesa() {
  ...
  ranking.slice(0, 7).forEach((item, index) => {
    container.innerHTML += `
      <div class="top-item">
        <div>
          ${item.desa}
...
const toggleTeamCompare =
  document.getElementById("toggleTeamCompare");

const teamCompareSection =
  document.getElementById("teamCompareSection");
...
function applyTableSettings() {
  const showTeamCompare =
    localStorage.getItem("showTeamCompare") !== "false";
  ...
  teamCompareSection.style.display =
    showTeamCompare
      ? "block"
      : "none";
...
toggleTeamCompare.addEventListener("change", () => {
